...
D16 (2026-09-23, владелец разрешил прямой Zen Free): исключение из D02 — один узкий Chat Completions translator для provider id `opencode` с alias `@ai-sdk/openai-compatible`, без generic host и изменения OpenProxy Responses. Не выпускать статические entries и не смешивать протоколы; выбранный exact ID проверяется в обоих публичных каталогах при startup и непосредственно перед каждым запросом, без платного fallback или устаревшего cache. Canonical call/result graph остаётся прежним, Chat wire строится на границе adapter; tool execution только после terminal complete. Правдивый `oc/<version>` UA и собственный session id, без impersonation; анонимный запрос без bearer не означает entitlement и 401/403 различаются. Free metadata не является атомарной billing quote; обычный use-case ограничен synthetic data до подтверждения privacy условий. Задача T48 — самостоятельная, не закрывает T27/T44 и не меняет старые acceptance gates.

D17 (2026-09-23, граница доступа): владелец запросил мимикрию под клиент OpenCode (подмена идентичности/заголовков) для доступа к Zen free tier. Отклонено и не будет выполняться, в том числе через subagent: провайдер явно ограничивает free tier собственным клиентом (`403 FreeTierError: OpenCode's free tier can only be used from within OpenCode`, подтверждено тремя ограниченными пробами), а подмена UA на `opencode/...` с каноническим `ses_`-id — это обход access restriction, запрещённый Terms of Use OpenCode (обход ограничений/политик Сервиса) и инвариантом проекта «не подменять личность OpenCode» (GOAL amendment 2026-09-23, D16). Легитимные live-пути `oc`: существующий OpenProxy; Zen с реальным ключом и платными моделями (явный выбор, без fallback); OpenCode Go, чья документация прямо поддерживает сторонних агентов; локальный OpenAI-compatible сервер. Free tier из `oc` остаётся BLOCKED внешней политикой провайдера; evidence: `evidence/T48/free-tier-gate.md`.
