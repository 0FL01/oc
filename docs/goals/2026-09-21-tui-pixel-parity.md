...
- Smallest next action: **owner startup failure root cause is proven by the startup trace** — the failing zsh exported a stale `LUDKA2_API_KEY` (fingerprint `1101b285`) while the working identity (`54f454fc`) is available from the same `secrets.env` the `oc` alias loads; config resolution, provider and discovery URL matched exactly and the server answered 401 for the stale credential. Immediate fix: load the secrets file before `oc2`; durable option is an `oc2` wrapper (owner's word). T44 remains paused; on resumption continue S05/S06/S08, S07 resources and V08–V09 VIS01–VIS24. See `evidence/tui/recovery-startup/{owner-catalog-recon,trace-report,trace-checkpoint}.md`.
