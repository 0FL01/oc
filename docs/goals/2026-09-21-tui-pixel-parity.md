...
- Smallest next action: V07 source admission before reads and full safety/resource qualification including real in-flight MCP cancellation. V06b now projects public reasoning and actual owner-owned tool/partial-patch output across restart, but original/native paired frames remain unequal. Typed release-startup diagnostics are verified only against isolated config; the user's real-profile cause is not known. V04 missing backend/service capabilities remain unavailable, not fabricated controls.
