...
- Smallest next action: V07c remaining S03–S08 safety/resource qualification, including the separate `{file:}` substitution ancestor-swap path. V07b actual remote/stdio call cancellation, durable unknown outcome and in-process remote retry quarantine across reload/Location were checked via raw PTY/application effects (`evidence/tui/recovery-v07b/checkpoint.md`); remote effects after process restart still require external reconciliation. Original/native frames remain unequal. The owner's real-profile release-startup cause is not known; missing backend/service capabilities remain unavailable, not fabricated controls.
