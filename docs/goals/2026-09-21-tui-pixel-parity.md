...
- Smallest next action: owner release now reports **catalog HTTP 401/403** under their zsh, not missing exported credentials; see `evidence/tui/recovery-startup/catalog-smoke-checkpoint.md`. This process's inherited product credential returned catalog 200 with the selected ID under both native and JS-style User-Agent; a fresh actual-profile release reached Home, but equality with the owner's zsh environment and its status are not established. The new value-free startup category differentiates 401 from 403 without bypassing discovery; that same-shell status or provider-side status-only trace is the next owner-specific input. Independently continue V07 S03/S05/S06/S08, S07 resources, VIS and missing genuine actions.
