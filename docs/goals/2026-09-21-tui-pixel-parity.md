...
- Smallest next action: pause further T44 implementation after committing the V07e S04 check; investigate the owner's still-failing real `target/release/oc` startup first when work resumes. The owner has authorized `~/.config/opencode` inspection, but this evidence was not collected; prior isolated-fixture startup success does not establish the cause. V07 S03/S05/S06/S08 and S07 resources, VIS, and missing genuine actions remain open. V07e actual raw-PTY sentinel check: `evidence/tui/recovery-v07e/checkpoint.md`.
...
- Next: V06a Markdown and V06b public reasoning/tool/patch/replay verified locally with actual application effects and paired original/native captures; all whole-frame comparisons still DIFFERENT. See `evidence/tui/recovery-v06{a,b}/checkpoint.md`; VIS13–VIS17/R4 remain unverified. V07a–c source/MCP negative checks and V07e one raw-PTY terminal-control path have factual checkpoints; remaining V07 safety/resources, V08–V09 and missing backend/service actions remain open. **Priority on resume:** diagnose owner's failing release launch with newly authorized profile access, before more T44 slices. Isolated release startup with synthetic config does not prove the owner's profile; positive live visual inspection is not a waiver. Native autoaccept remains Unsupported (`evidence/tui/recovery-v02/auto-capability.md`).
