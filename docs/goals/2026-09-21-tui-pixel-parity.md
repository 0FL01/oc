...
- Smallest next action: V07b real remote/stdio in-flight MCP `tools/call` cancellation and owner cleanup, followed by V07c remaining safety/resource qualification. V07a fixed before-read discovered-root and nested definition/instruction admission with actual binary/PTY effects (`evidence/tui/recovery-v07a/checkpoint.md`); the separate `{file:}` ancestor-swap path still needs qualification. Original/native frames remain unequal. The owner's real-profile release-startup cause is not known; missing backend/service capabilities remain unavailable, not fabricated controls.
