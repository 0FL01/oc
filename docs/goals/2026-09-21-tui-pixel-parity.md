...
- Smallest next action: **owner requested T44 pause after the S03 checkpoint and a priority startup RECON**; a bounded redacted per-launch startup trace is now delivered (see `evidence/tui/recovery-startup/trace-{report,checkpoint}.md`). The owner runs the rebuilt release in the failing shell with `OC_STARTUP_TRACE_FINGERPRINT=1` and compares the credential fingerprint/config root against the working smoke (`fingerprint=54f454fc`, root `/home/opencode/.config/opencode`); the trace names the exact failing stage and typed category. S03 actual PTY effects are checked in `evidence/tui/recovery-v07-s03/{report,checkpoint}.md`; S05/S06/S08, S07 resources, VIS and missing genuine actions remain open pending resumption.
