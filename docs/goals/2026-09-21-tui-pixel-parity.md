...
- Smallest next action: the owner's zsh **did** export both selected provider variables and still received Configuration load failed, disproving the earlier missing-export explanation for their failure. A freshly built release now classifies discovery authorization, HTTP, network, invalid/empty catalog and selected-ID absence separately (`evidence/tui/recovery-startup/discovery-checkpoint.md`). The owner's private catalog outcome is not available to this agent and must be identified by the new static category under that shell before changing model/config; do not treat successful startup with a different key as proof. V07 S03/S05/S06/S08, S07 resources, VIS and missing genuine actions remain open.
...
- Next: V06a Markdown and V06b public reasoning/tool/patch/replay verified locally with actual application effects and paired original/native captures; all whole-frame comparisons still DIFFERENT. See `evidence/tui/recovery-v06{a,b}/checkpoint.md`; VIS13–VIS17/R4 remain unverified. V07a–c source/MCP negative checks and V07e one raw-PTY terminal-control path have factual checkpoints; remaining V07 safety/resources, V08–V09 and missing backend/service actions remain open. Owner-reported exported credentials refute the prior missing-export root-cause claim; that controlled reproduction remains valid only as a negative test. Static typed discovery categories and validated real-HTTP/PTy fixtures in `evidence/tui/recovery-startup/discovery-checkpoint.md` narrow the remaining owner-key-dependent diagnosis. Positive live visual inspection is not a waiver; native autoaccept remains Unsupported (`evidence/tui/recovery-v02/auto-capability.md`).
