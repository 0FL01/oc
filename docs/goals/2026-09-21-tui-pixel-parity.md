...
- Smallest next action: V06 Markdown tables and live/replay reasoning/tool/diff cards with actual provider/tool effects and paired original/Rust captures. Typed release-startup diagnostic slice is verified only against isolated config; the user's real-profile cause is not known. V04 missing backend/service capabilities remain unavailable, not fabricated controls.
...
- Next: V05 editor and paste-chip PTY verified locally, but paired whole-frame comparators remain DIFFERENT and VIS11–VIS12 unverified (`evidence/tui/recovery-v05/checkpoint.md`). Isolated fresh release-binary startup works with valid static config; categorized startup and Location switch failures and terminal recovery qualified in `evidence/tui/recovery-startup/checkpoint.md`. Owner's real-profile failure cause remains unknown; neither private `.opencode/` nor credentials were inspected. V06–V09 and genuine missing backend/service actions remain open; R3/R4 unverified. Positive live visual inspection is not a waiver. Native autoaccept remains Unsupported (`evidence/tui/recovery-v02/auto-capability.md`).
