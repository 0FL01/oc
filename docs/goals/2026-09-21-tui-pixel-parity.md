...
- Smallest next action: V07 remaining S03–S06/S08 negative qualification and S07 measured resources. V07c reproduced and fixed the separate `{file:}` substitution ancestor-swap through actual binary/PTY (`evidence/tui/recovery-v07c/checkpoint.md`). V07b checked remote/stdio cancellation and in-process retry quarantine, but remote effects after process restart still require external reconciliation. Original/native frames remain unequal; the owner's real-profile release-startup cause is not known and missing backend/service capabilities remain unavailable, not fabricated controls.
...
- Next: V06a Markdown and V06b public reasoning/tool/patch/replay verified locally with actual application effects and paired original/native captures; all whole-frame comparisons still DIFFERENT. See `evidence/tui/recovery-v06{a,b}/checkpoint.md`; VIS13–VIS17/R4 remain unverified. V07a admitted roots/definitions, V07b MCP cancel/unknown outcomes and V07c file references tested through actual binary/PTy, see their checkpoints. Remaining V07 safety/resource gates, V08–V09 and genuine missing backend/service actions are open. Isolated release-binary startup and safe diagnostics qualified in `evidence/tui/recovery-startup/checkpoint.md`; owner real-profile cause remains unknown and private config was not accessed. Positive live visual inspection is not a waiver; native autoaccept remains Unsupported (`evidence/tui/recovery-v02/auto-capability.md`).
