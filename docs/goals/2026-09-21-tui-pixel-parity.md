...
- Smallest next action: owner must verify/export the selected provider's `LUDKA2_API_KEY` in the **same interactive shell** that launches `target/release/oc`, without printing the value. Real-profile release PTY now starts with an inherited nonempty key and fails with the typed **Selected provider credential missing** screen if it is removed (`evidence/tui/recovery-startup/credential-checkpoint.md`). The owner's interactive environment was not accessible to this API process; do not claim it was measured. After confirming that handoff, V07 S03/S05/S06/S08, S07 resources, VIS and missing genuine actions remain open.
...
- Next: V06a Markdown and V06b public reasoning/tool/patch/replay verified locally with actual application effects and paired original/native captures; all whole-frame comparisons still DIFFERENT. See `evidence/tui/recovery-v06{a,b}/checkpoint.md`; VIS13–VIS17/R4 remain unverified. V07a–c source/MCP negative checks and V07e one raw-PTY terminal-control path have factual checkpoints; remaining V07 safety/resources, V08–V09 and missing backend/service actions remain open. Real-profile startup: with the selected credential inherited, release reaches Home on default data; removing that variable alone reproduces the owner's old Configuration error, now typed and actionable (`evidence/tui/recovery-startup/credential-checkpoint.md`). Actual interactive-shell export remains unverified; positive live visual inspection is not a waiver. Native autoaccept remains Unsupported (`evidence/tui/recovery-v02/auto-capability.md`).
