...
fn allow_all() -> BTreeMap<String, Permission> {
    ["read", "apply_patch", "bash", "webfetch", "skill", "compress"]
        .into_iter()
...
fn measure_turn(pairs: usize) -> (usize, String) {
    ...
    let body = bodies.lock().expect("bodies").first().cloned().unwrap_or_default();
    (peak, body)
}
...
async fn aud33_payload_above_64k_within_budget_is_complete() {
    ...
    harness.db.append_message("work", "user", &big).expect("user");
    harness
        .db
        .append_message("work", "assistant", "ack")
        .expect("assistant");
    harness.db.append_message("work", "user", &big).expect("user2");
    harness
        .db
        .append_message("work", "assistant", "ack")
        .expect("assistant2");
    ...
    let report = runtime
        .run_turn(params("work", &prompt, &harness, provider_of(&base), &cancel))
        .await
        .expect("turn");
    assert_eq!(report.status, TurnStatus::Completed);
    let body = bodies
...
async fn aud34_tool_output_beyond_preview_is_retrievable() {
    ...
    let cancel = AtomicBool::new(false);
    let report = runtime
        .run_turn(params("work", "read the facts", &harness, provider_of(&base), &cancel))
        .await
        .expect("turn");
    assert_eq!(report.status, TurnStatus::Completed);
    ...
    let ops = harness.db.list_tool_ops("work").expect("ops");
    let row = ops.last().expect("one op");
    assert_eq!(row.name, "read");
    let stored = row.output.clone().unwrap_or_default();
    assert!(
        stored.contains("NEEDLE-AFTER-2048"),
