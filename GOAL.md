...
## Owner scope amendment (2026-09-23): direct Zen Free Chat

По явному решению владельца добавлен ограниченный native provider `opencode` **только для официального Zen Free `/zen/v1/chat/completions`**. Это исключение из исторических запретов Chat/других native families в строках 9, 53 и 63; Responses/OpenProxy и его A04/PROV08/E2E02 остаются обязательными и не подменяются Zen evidence. Без Messages, Responses-Zen, Go, System One, npm/JS host или списка production model IDs в коде. Eligibility — пересечение актуальных моделей `models.dev/api.json` и `opencode.ai/zen/v1/models`, с явно нулевой ценой во всех поддерживаемых категориях, Chat protocol, text/tools и live ID; неизвестное/устаревшее/платное — отказ без генерации. При каждом generation HTTP заново проверять eligibility, включая title/child. Только правдивые headers `oc/<version>` и собственный стабильный session id; без ключа — официальный анонимный маркер `Authorization: Bearer public` (как upstream plugin), с реальным ключом — его bearer. `public` не является правом доступа: решение принимает gateway. Личность OpenCode не подменять; подтверждено ограниченными пробами, что free tier отказывает любому не-OpenCode клиенту (`FreeTierError`), поэтому реальные прогоны `oc` остаются на OpenProxy, платном Zen key или Go. Дальнейшие условия проверки: [docs/goals/2026-09-23-zen-free-chat.md](docs/goals/2026-09-23-zen-free-chat.md).
